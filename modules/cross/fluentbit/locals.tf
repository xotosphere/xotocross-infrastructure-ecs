...
locals {
  ...
  xtcross-container-fluentbit = jsondecode(templatefile("${path.module}/aws/task-container.tpl", {
    ...
    xtcross-container-environment = jsonencode([
      ...
      { name = "LOKI_HOST", value = var.environment == "production" ? "loki.${var.xtcross-service-name}.${var.xtcross-domain-name}.com" : "loki.${var.xtcross-service-name}.${var.environment}.${var.xtcross-domain-name}.com" },
  ...
  xtcross-listener-hostlist = concat(var.xtcross-listener-hostlist, var.xtcross-enable-monitor ? (var.environment == "production" ? ["fluentbit.${var.xtcross-service-name}.${var.xtcross-domain-name}.com"] : ["fluentbit.${var.xtcross-service-name}.${var.environment}.${var.xtcross-domain-name}.com"]) : [])
