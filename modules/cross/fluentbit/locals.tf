...
locals {
  xtcross-container-fluentbit = jsondecode(templatefile("${path.module}/aws/task-container.tpl", {
    ...
    xtcross-container-environment = jsonencode([
      ...
      { name = "LOKI_HOST", value = "${var.prefix}-monitor-loki.${var.environment}.local", },
      ...
      { name = "LOG_GROUP", value = "/aws/ecs/${var.prefix}-${var.xtcross-service-name}-${var.environment}-log" },
    ])
    ...
    xtcross-logconfiguration                = jsonencode({ logDriver = "awslogs", options = { "awslogs-group" = "/aws/ecs/${var.prefix}-${var.xtcross-service-name}-${var.environment}-log", "awslogs-region" = var.region } })
