...
locals {
  ...
  xtcross-listener-hostlist = concat(var.xtcross-listener-hostlist, var.xtcross-enable-monitor ? (var.environment == "production" ? ["fluentbit.${var.xtcross-domain-name}.com"] : ["fluentbit.${var.environment}.${var.xtcross-domain-name}.com"]) : [])
