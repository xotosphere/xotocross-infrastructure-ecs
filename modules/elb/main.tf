...
data "external" "certificate" {
  program = ["bash", "-c", "arn=$(aws acm list-certificates --region eu-west-3 | jq -r '.CertificateSummaryList[] | select(.DomainName == \"xotosphere.com\" and .Status == \"ISSUED\") | .CertificateArn' | head -n 1); echo '{\"arn\": \"$arn\"}'"]
}
