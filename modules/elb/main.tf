...
data "external" "certificate" {
  program = ["bash", "-c", "aws acm list-certificates --region eu-west-3 | jq -r '.CertificateSummaryList[] | select(.DomainName == \"xotosphere.com\" and .Status == \"ISSUED\") | .CertificateArn' | head -n 1"]
}
...
resource "aws_lb_listener" "xtcross-http-listener" {
  ...
  certificate_arn = data.external.certificate.result == "" ? null : data.external.certificate.result

  protocol = data.external.certificate.result == "" ? "HTTP" : "HTTPS"
