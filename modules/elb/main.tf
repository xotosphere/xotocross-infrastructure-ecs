...
resource "aws_lb_listener" "xtcross-https-redirection" {
  count             = (local.hasCert && length(data.aws_lb_listener.xtcross-https-redirection) == 0) ? 1 : 0
  load_balancer_arn = data.aws_lb.xtcross-loadbalancer.arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_lb_listener" "xtcross-http-listener" {
  count             = length(data.aws_lb_listener.xtcross-http-listener) == 0 ? 1 : 0
  load_balancer_arn = data.aws_lb.xtcross-loadbalancer.arn
...
resource "aws_lb_listener_rule" "xtcross-http-listener-rule" {
  ...
  listener_arn = length(data.aws_lb_listener.xtcross-http-listener) > 0 ? data.aws_lb_listener.xtcross-http-listener[0].arn : aws_lb_listener.xtcross-http-listener[0].arn
